"""
  ...
  @FileName     : 811.子域名访问计数.py
  @Description  : 一个网站域名，如"discuss.leetcode.com"，包含了多个子域名。作为顶级域名，常用的有"com"，
...
class Solution:
    ...
    def subdomain_visits(cpdomains: List[str]) -> List[str]:
        ...
        for record in cpdomains:
            counts, addr = record.split()
            counts = int(counts)
            for i, char in enumerate(addr):
                if char == '.':
                    lookup[addr[i+1:]] += counts
            lookup[addr] += counts

        return [f"{v} {k}" for k, v in lookup.items()]
...
if __name__ == '__main__':
    ...
    for tc in test_cases:
        print(Solution.subdomain_visits(tc))
