...
class Solution:
    ...
    def is_alien_sorted(words: List[str], order: str) -> bool:
        sorted_words = sorted(words, key=lambda x: [order.index(s) for s in x])
        return sorted_words == words

    # 48ms, 13.6MB
    @staticmethod
    def is_alien_sorted_v2(words: List[str], order: str) -> bool:
        dic = {char: i for i, char in enumerate(order)}
        sorted_words = sorted(words, key=lambda x: [dic[s] for s in x])
        return sorted_words == words
...
if __name__ == '__main__':
    ...
    for tc in test_cases:
        print(Solution.is_alien_sorted(*tc))
        print(Solution.is_alien_sorted_v2(*tc))
